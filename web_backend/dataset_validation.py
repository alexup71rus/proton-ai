...
from web_backend.enum_values import enum_is_supported
from web_backend.enum_values import enum_output_values
...
def build_preview_lines(content: str, limit: int = 5) -> list[dict[str, Any]]:
    ...
    for index, raw_line in enumerate(content.splitlines(), start=1):
        if not raw_line.strip():
            continue
        rows.append({"line_number": index, "raw": raw_line})
        if len(rows) >= limit:
            break
    return rows
...
def _parse_compact_row(row: dict[str, Any], line_number: int, add_issue) -> tuple[list[dict[str, Any]], str, dict[str, Any]] | None:
    tools = row.get("tools")
    user = row.get("user")
...
def _parse_legacy_row(row: dict[str, Any], line_number: int, add_issue) -> tuple[list[dict[str, Any]], str, dict[str, Any]] | None:
    tools = row.get("tools")
...
def validate_dataset_content(content: str, max_issues: int = 25) -> dict[str, Any]:
    issue_count = 0
    ...
    for line_number, raw_line in enumerate(content.splitlines(), start=1):
        ...
        tools, _user, payload = normalized_row
